...
function TaskItem({ task, onUpdate, onDelete }) {
  ...
  // Définition de la progression selon le statut
      ...
      {editing ? (
        <div style={{ display: "flex", gap: 8, flexDirection: "column" }}>
          ...
          <input
            ...
            onChange={e => setLocalTask({ ...localTask, user: e.target.value })}
            placeholder="Utilisateur"
          />
          <input
            value={localTask.value}
            onChange={e => setLocalTask({ ...localTask, value: e.target.value })}
            placeholder="Valeur"
          />
          <select
          ...
          <input
            ...
            onChange={e => setLocalTask({ ...localTask, estimate: Number(e.target.value) })}
        ...
        <div style={{ cursor: "pointer" }} onClick={() => setEditing(true)}>
          <strong>{task.title}</strong> - {task.user} - {task.value} - {task.status} - {task.estimate}j
